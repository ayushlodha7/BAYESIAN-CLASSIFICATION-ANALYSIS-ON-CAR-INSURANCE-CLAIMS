...
post <- stan_glm(reg_formula, data = train_Data,
...
classification_summary(model = post,data = train_Data,cutoff = 0.5)
classification_summary(model = post,data = test_Data,cutoff = 0.5)

pp_check(post)
pp_check(post,plotfun = "hist",nreps=5)
pp_check(post, plotfun = "error_binned")
